"""Correctness tests for AdaLayerNormFwdOp (NPU).

Adapted from TileOPs ``tests/ops/test_ada_layer_norm.py`` (the GPU file is
entirely AdaLN non-gated coverage; the gated AdaLN-Zero tests live in the
separate GPU ``test_ada_layer_norm_zero.py`` and are not migrated).

NPU adaptations (T1-T4):

- T1: manual tensor creation uses the device backend (``_device()``)
  instead of hard-coded ``device="cuda"``.
- T2: no ``@pytest.mark.skipif(not torch.cuda.is_available(), ...)``
  guards in the GPU file -- nothing to remove.
- T3: the ``threads`` key in the direct-kernel config override is removed
  (K9); the kernel-direct tests otherwise keep the GPU param values.
- T4: no ``tune`` parametrization / autotune try-except in the GPU file --
  nothing to remove.
"""
...
from tileops.kernels.norm.ada_layer_norm.ada_layer_norm import _should_use_cp_async
    ...
    PARAMS = [
        (
            ...
            [
                ...
                pytest.param(
                    1024, 4096, torch.float32, marks=[pytest.mark.smoke, pytest.mark.packaging]
                ),
...
def test_ada_layer_norm_async_copy_handles_row_tail() -> None:
    ...
    test = AdaLayerNormTest(m, n, dtype)
    inputs = test.gen_inputs()
    # T3/K9: GPU config {"block_m": block_m, "threads": 128} -> threads
    # removed on NPU.
...
def test_ada_layer_norm_3d(batch: int, seq: int, hidden: int, dtype: torch.dtype) -> None:
    ...
    # T1: device via the device backend (was ``device="cuda"``).
    device = _device()

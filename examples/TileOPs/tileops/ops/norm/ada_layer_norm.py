"""Adaptive LayerNorm (AdaLN) operator (NPU-adapted).

Adaptation from GPU (TileOPs) ``tileops/ops/norm/ada_layer_norm.py``:

- O1: ``x.is_cuda`` / ``scale.is_cuda`` / ``shift.is_cuda`` device checks
  -> ``backend.is_device_tensor(...)`` for all three inputs.
- O2: no hard-coded ``device="cuda"`` (device resolved via the backend).
- O3: ``tune`` parameter removed from ``__init__`` and ``_get_kernel``;
  the kernel constructor takes
  ``(M, N, eps, dtype, has_gate=False, config=None)``.
- O6: op-specific flow preserved unchanged -- validation, 3-input
  ``contiguous().reshape(-1, N)``, kernel cache keyed on
  ``(M, N, dtype, device_index)``, ``y.reshape(orig_shape)``, and the
  roofline formula (device-agnostic).
"""
...
class AdaLayerNormFwdOp(Op):
    ...
    def forward(
        ...
        Args:
            x: Input tensor of shape ``(*leading, N)`` on the target device.
            scale: Per-token scale tensor of shape ``(*leading, N)`` on the
                target device.
            shift: Per-token shift tensor of shape ``(*leading, N)`` on the
                target device.
        ...
        Raises:
            ValueError: If tensors are not on the target device, dtypes
                mismatch, or shapes are incompatible with the configured
                dimensions.
        """
        # O1: device checks via the device backend (was ``.is_cuda``).
        backend = get_device_backend()
        if not backend.is_device_tensor(x):
            raise ValueError(f"x must be a {backend.name} tensor")
        if not backend.is_device_tensor(scale):
            raise ValueError(f"scale must be a {backend.name} tensor")
        if not backend.is_device_tensor(shift):
            raise ValueError(f"shift must be a {backend.name} tensor")
        expected_dtype = self._committed_dtype
        if expected_dtype is not None and x.dtype != expected_dtype:
            raise ValueError(f"Expected x.dtype {expected_dtype}, got {x.dtype}")

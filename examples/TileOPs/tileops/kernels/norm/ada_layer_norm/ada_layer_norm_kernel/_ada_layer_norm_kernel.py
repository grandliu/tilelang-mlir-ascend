...
# Measured multi-buffered UB cost per element of the (block_m, N)
# resident block (attempt-1 finding, see module Implementation Notes):
# when the persistent serial loop is active (num_local_tasks >= 2) the
# auto-multi-buffer pass materializes ~20 B/elem on BOTH dtype paths
# (fp16/bf16 transit: 2 instances x (stage 2B + x_f32 4B + sq_f32 4B);
# fp32 direct: 2.5x of (x_f32 4B + sq_f32 4B)). Compile-probe overflow
# messages read 245760 B = 20*3*4096 (fp32 bm=3 N=4096) and
# 240000 B = 20*4*3000 (fp32 bm=4 N=3000, exact match). DESIGN.md
# section 4.5 budgeted 192 KB / 1.7 (~115 KB resident); the measured
# factors are 2.0 (fp16/bf16) / 2.5 (fp32), so the budget is expressed
# in resident elements: 192 KB / 20 B = 9830, kept at 9216 (= the
# design's largest sanctioned config bm=8 x N=1152, >= 12 KB margin).
# num_local_tasks == 1 (no serial iterations) does not inflate; the
# guard stays unified and conservative for simplicity.
_UB_BYTES_PER_ELEM = 20
_UB_MAX_ELEMS = 9_216
...
# Kernel
# ---------------------------------------------------------------------------
def _default_block_m(N):
    """UB-budget default block_m (DESIGN.md section 4.5 formula, with the
    measured 20 B/elem multi-buffered budget; see Constants)."""
    return min(_BLOCK_M_CAP, max(1, _UB_MAX_ELEMS // N))
...
def _ada_layer_norm_kernel(M, N, eps, dtype, has_gate=False, use_cp_async=False):
    ...
    block_m_default = _default_block_m(N)
    ...
    def _func(block_m=block_m_default):
        # Host-side UB budget guard (DESIGN.md sections 3.3 and 9.2, with
        # the measured 20 B/elem multi-buffered budget; see Constants).
        # ValueError (not a bare assert) so the guard survives `python -O`.
        if block_m < 1:
            raise ValueError(f"block_m must be positive, got block_m={block_m}")
        if block_m * N > _UB_MAX_ELEMS:
            raise ValueError(
                f"block_m={block_m} exceeds the UB budget for N={N} "
                f"(max {_UB_MAX_ELEMS} resident elements at "
                f"{_UB_BYTES_PER_ELEM} B/elem multi-buffered, "
        ...
        if use_fp32_transit:
            ...
            def main(
                ...
                with T.Kernel(num_kernels, is_npu=True) as (cid, _):
                    # UB buffers (DESIGN.md section 4.3), allocated once
                    # per core and reused across serial tasks:
                    #   stage : x -> scale -> shift -> rounded result (4 roles)
                    #   x_f32 : x fp32 -> d (kept, O3) -> final fp32 result
                    #   sq_f32: d^2 -> scale fp32 -> shift fp32 (3 roles)
                    stage = T.alloc_shared((block_m, N), dtype)
                    ...
                    for i in T.serial(num_local_tasks):
                        ...
                        if block_id < num_logical:
                            ...
                            # --- 3. Centered two-pass variance ---
                            # d is kept for the epilogue (O3); exact N,
                            # no pad accounting (O4 identity).
                            ...
                            T.vcast(x_f32, stage, round_mode="rint")

                            # --- 5. Store (valid rows only) ---
                            T.copy(
                                stage[0:real_m, 0:N],
            ...
            def main(
                ...
                with T.Kernel(num_kernels, is_npu=True) as (cid, _):
                    # fp32 path (DESIGN.md section 4.3): no transit
                    # buffers, same-dtype direct copies, no vcast.
                    x_f32 = T.alloc_shared((block_m, N), "float32")
                    sq_f32 = T.alloc_shared((block_m, N), "float32")
                    s1 = T.alloc_shared((block_m, 1), "float32")
                    mean = T.alloc_shared((block_m, 1), "float32")
                    ...
                    for i in T.serial(num_local_tasks):
                        ...
                        if block_id < num_logical:
                            ...
                            # Same-dtype GM -> UB copy (fp32 direct path).
                            T.copy(
                                x[off_m : off_m + real_m, 0:N],
                                x_f32[0:real_m, 0:N],
                            )
...
def run_L0():
    ...
    # L0-1 note: the design table lists bm=3 for fp32 N=4096, which the
    # measured 20 B/elem multi-buffer budget rejects (3*4096 = 12288 >
    # 9216; compile-probe overflow 245760 B > 192 KB); per the DESIGN.md
    # section 9.1 contingency ("drop one step in the section 4.5 table")
    # the case runs at bm=2.
    cases = [
        (1024, 4096, "float32", 2, "L0-1"),  # aligned fp32 direct path
        ...
        (64, 1152, "float32", 8, "L0-9/fp32"),  # manifest smoke-dit
        (64, 1152, "float16", 8, "L0-9/fp16"),
        (64, 1152, "bfloat16", 8, "L0-9/bf16"),
        (1024, 1152, "float16", 8, "L0-10"),  # manifest dit-xl-2
        (2048, 4096, "float16", 2, "L0-11"),  # manifest llama-prefill
        (1, 4096, "bfloat16", 2, "L0-12"),  # manifest llama-decode (M=1)

"""PyTorch reference implementations for the mamba (Mamba-2 / SSD) family.

Ported from TileOPs ``tileops/testing/mamba2_reference.py`` -- SSDChunkScanFwdOp
subset only (the shared GPU module also provides ``mamba2_fwd_ref`` for
Mamba2FwdOp, which is not migrated yet).

Pure PyTorch, device-agnostic: every tensor is placed/derived from the
inputs' own device, so no W1-style device adaptation is required.
"""
...
__all__ = ["ssd_chunk_scan_fwd_ref"]

...
Autotune 属于 Phase 2 的一个实验分支，通常放在结构性优化之后执行。
...
- 若搜索 `num_cores`，搜索空间必须覆盖 AI Core Count 附近、2x/4x/6x/8x AI Core Count、任务并发甜点区、低 imbalance 候选和 flat-grid 端点，不能只测单点。
...
不要把 autotune 理解为自动完成所有性能优化。它只是在给定搜索空间中选择最快配置。

...
| 分核策略（物理核数适配） | §5 含分核策略三要素：逻辑核数计算（`ceil(M/block_M) × ceil(N/block_N)`）、物理核数及来源（`NPUUtils.get().get_aicore_num()` 实查——Cube/混合算子直接用返回值、纯 Vector 算子核数翻倍；记录查询代码与实际返回值，禁止文档假设/经验值替代）、规模判定与分核方案（逻辑核数 ≤ 物理核数给"无需适配"依据 / 中等规模对齐物理核整数倍（按实查核数取 1×/2×/3×）/ 极大规模核内 `T.serial` 串行且循环边界静态）；核数与 block 取值自洽（依据 docs/开发指南.md §3.3） | 逻辑核数远超物理核数却无核内串行设计；物理核数非实查（无 NPUUtils 查询记录或使用文档假设/经验值）；启动 21 核类负载不均未处理；核内串行任务数依赖动态 shape |

...
def vec_deinterleave(block_M, block_N, channel_nums, dtype="float16"):
    ...
    def sliceDeinterleaveExp(
        ...
        with T.Kernel(BLOCK_SIZE, is_npu=True) as (cid, _):
            ...
            O0_VEC = T.alloc_ub((block_M, block_N_half), dtype)
            O1_VEC = T.alloc_ub((block_M, block_N_half), dtype)
            T.copy(A, A_VEC)
    ...
    return sliceDeinterleaveExp


@pytest.mark.parametrize("dtype", DTYPES)
def test_vec_deinterleave_exp(dtype):
    M, N = 32, 32
